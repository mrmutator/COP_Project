...
For the evaluation step is mandatory to utilize labelled data, for this reason, we train and evaluate our classifiers on the Switchboard corpus. We divide the dataset into a training and a test set, containing $\%$ and $\%$ of the total length, respectively. \lau{add percentages}
...
Just like \newcite{stolcke200} we will model a dialog as an HMM. The hidden states will be the dialog acts and the observed quantities the uttered words and their speaker. To find the most likely tag for a single utterance we will find the best dialog act tag $t$ for an utterance using equation \ref{eq:maxtag}.
