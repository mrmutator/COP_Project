...
In our experiments, we also use a coarser tagset where we manually map the original 42 tags into 8 classes. The mapping is described in \roger{appendix or where?}.
The motivation for this coarser tag set is we expect classifiers to more likely confuse two utterances that are close in \david{form/meaning}, for example a yes-no-question and an action-directive.
