...
%\end{itemize}
\subsection{Baseline}
As a baseline we represent each utterance as a bag of words. We represent each utterance as a sparse vector, were every dimension maps to a word in the vocabulary. We then train a Naive Bayes model and a K-Nearest Neighbor classifier on the dialog act tagging task, both for the full tag set of 42 tags as well as on the aggregated tag set \david{How many tags}. The results can be seen in Table \david{make table with results} and \david{a seperate table for aggregated tag set pherhaps?} respectively. \david{Report best baseline explicitly here}
